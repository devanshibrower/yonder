...
export default function LunarCalendar({ selectedDay: controlledDay, onDaySelect, currentLabel }: LunarCalendarProps = {}) {
  ...
  const [internalDay, setInternalDay] = useState<number>(dayOfYear);
  const resolvedDay = controlledDay ?? internalDay;
  ...
  // Ref to always hold latest onDaySelect — avoids stale closure in mount-only effects
  const onDaySelectRef = useRef(onDaySelect);
  onDaySelectRef.current = onDaySelect;
  const controlledDayRef = useRef(controlledDay);
  controlledDayRef.current = controlledDay;

  // Stable helper that never goes stale — safe to call from mount-only effects
  const updateDay = useRef((day: number) => {
    if (onDaySelectRef.current) onDaySelectRef.current(day);
    if (controlledDayRef.current === undefined) setInternalDay(day);
  }).current;
  ...
  selectedDayRef.current = resolvedDay;
  ...
  useEffect(() => {
    const displayDay = hoveredDay ?? resolvedDay;
    targetPhaseRef.current = getMoonPhase(displayDay);
  }, [hoveredDay, resolvedDay]);
  ...
  return (
    <div className="flex flex-col items-center gap-0">
        ...
        <canvas
          ...
          onKeyDown={(e) => {
            if (e.key === "ArrowRight" || e.key === "ArrowDown") {
              e.preventDefault();
              setHoveredDay(null);
              const next = resolvedDay >= 365 ? 1 : resolvedDay + 1;
              updateDay(next);
            } else if (e.key === "ArrowLeft" || e.key === "ArrowUp") {
              e.preventDefault();
              setHoveredDay(null);
              const next = resolvedDay <= 1 ? 365 : resolvedDay - 1;
              updateDay(next);
            }
          }}
        />
      ...
      <div className="text-center">
        ...
        <button
          onClick={() => updateDay(dayOfYear)}
          disabled={resolvedDay === dayOfYear}
        ...
        {currentLabel && (
          <div className="mt-3 font-sans text-[10px] uppercase tracking-[0.12em] text-zinc-500 transition-all duration-700">
            {currentLabel}
          </div>
        )}
      </div>
